...
public class Symtab {
 ...
 public boolean add_entry(String myid, int myline, TK myVarOrConst) {
  ...
  // add var or const entry to current block
 public boolean add_entry(String myid, int myline, TK myVarOrConst, int lowerBound, int upperBound, int size) {
     Entry e = search_this_block(myid);
     if (e != null) {
         System.err.println(e.whatAreYou() +
                                " " + myid +
     ...
     Entry p = new Entry(myid, myline, myVarOrConst, lowerBound, upperBound, size);
     st.peek().add(p);
     return true;
 }
